...
function CopyLinkButton({ url }) {
  ...
  return (
    <button
      onClick={handleCopy}
      style={{ display: 'flex', alignItems: 'center', gap: '.75rem', padding: '.75rem 1rem', background: 'var(--navy)', border: '1px solid var(--border)', borderRadius: '8px', color: 'var(--off)', cursor: 'pointer', fontFamily: 'Barlow Condensed, sans-serif', fontWeight: 700, fontSize: '.9rem', textTransform: 'uppercase', letterSpacing: '.05em', width: '100%', textAlign: 'left' }}
    >
...
export default function ListingDetail() {
  ...
  const shareText = 'Check out "' + listing.title + '" on Coaches Pay Coaches'
  ...
  return (
    <div className="page-body">
      ...
      <div style={{ padding: '2.5rem 5%', maxWidth: '900px', margin: '0 auto' }}>
        <button className="btn btn-ghost" style={{ padding: '8px 16px', fontSize: '13px', marginBottom: '2rem' }} onClick={() => navigate('/marketplace')}>Back</button>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '2rem', alignItems: 'start' }}>
          <div>
            ...
            {listing.profiles?.full_name && (
              <div style={{ color: 'var(--muted)', fontSize: '.9rem', marginBottom: '1.5rem', cursor: 'pointer' }} onClick={() => navigate('/coach/' + listing.seller_id)}>
            ...
            <p style={{ color: 'var(--off)', lineHeight: 1.8, fontSize: '.95rem', marginTop: '0.5rem' }}>{listing.description}</p>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            <div className="cpc-card" style={{ padding: '1.25rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '.6rem' }}>
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: '.75rem', padding: '.75rem 1rem', background: '#000', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#fff', textDecoration: 'none', fontFamily: 'Barlow Condensed, sans-serif', fontWeight: 700, fontSize: '.85rem', textTransform: 'uppercase', letterSpacing: '.05em' }}
                >
                  𝕏 Share on X
                </a>
                <button
